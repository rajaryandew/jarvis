...
devices = httpx.get("https://api.smartthings.com/v1/devices",headers=headers).json()
...
def airconditioneraction(response:str):
    ...
    if(action == "turn_on"):
        ...
        httpx.post(f"https://api.smartthings.com/v1/devices/{ac_id}/commands",headers=headers,json=body)
        ...
        if(int(temp) < 16) or (int(temp) > 30):
            ...
            return
        body = createbody(capability="thermostatCoolingSetpoint",command=f"setCoolingSetpoint",arguments=int(temp))
        response = httpx.post(f"https://api.smartthings.com/v1/devices/{ac_id}/commands",headers=headers,json=body)
